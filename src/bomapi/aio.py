"""
Python async interface for the Australia BOM Weather API.
...
async def _make_request(url_path: str, *, params: Mapping[str, str] = None):
    async with httpx.AsyncClient() as client:
        response = await client.get(f"{API_BASE_URL}{url_path}", params=params)
...
async def location_search(search: str) -> Sequence[LocationResult]:
    """
    ...
    data = await _make_request(f"/locations", params={"search": search})
...
class Location:
    ...
    async def info(self) -> LocationInfo:
        """
        ...
        data = await _make_request(f"/locations/{self.geohash}")
        return LocationInfo.from_data(data)

    async def warnings(self) -> Sequence[LocationWarning]:
        """
        Identify warnings for the specified location
        """
        data = await _make_request(f"/locations/{self.geohash}/warnings")
        return [LocationWarning.from_data(item) for item in data]

    async def observations(self) -> LocationObservation:
        """
        Get observations made at the specified location
        """
        data = await _make_request(f"/locations/{self.geohash[:6]}/observations")
        return LocationObservation.from_data(data)

    async def forcast_daily(self) -> Sequence[DailyForecast]:
        """
        Daily forecast for the next seven days
        """

        data = await _make_request(f"/locations/{self.geohash[:6]}/forecasts/daily")
        return [DailyForecast.from_data(item) for item in data]

    async def forcast_3_hourly(self) -> Sequence[ThreeHourlyForcast]:
        """
        Three-hourly forecast for the next two days
        """

        data = await _make_request(f"/locations/{self.geohash[:6]}/forecasts/3-hourly")
